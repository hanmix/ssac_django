...
def sign_com(req): # 회원가입 후 동작하는 함수(함수만 동작)
  if req.method == "POST": # POST 로 넘어온 값이 있을 때, 실행
    ...
    if not (userid and username and pw and email and address and usergender):
        messages.info(req, '정보를 입력해주세요.')
        return redirect('signup')
    else :
        # Model을 객체로 불러와 POST로 넘어온 변수의 값 넣어주기
        user = FlowerUser(userid=userid, username=username, password=pw, useremail=email, useraddress=address, gender=usergender)
        user.save()
